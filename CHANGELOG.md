...
`SDK_VERSION` constant is kept in sync with `package.json` by the `npm version` lifecycle (`scripts/sync-version.mjs`) and guarded by `test/version.test.js`.

## [0.3.1] — 2026-08-14

### Fixed — published code no longer uses a JSON import attribute

`src/index.js` imported `package.json` with `with { type: "json" }` to derive
`SDK_VERSION` (since v0.2.3). Import attributes run on every runtime the SDK
targets, but a consumer's **bundler** must also parse them, and older bundlers
still in wide use hard-fail: wrangler 3's bundled esbuild rejects the file with
`Expected ";" but found "with"`, breaking `wrangler dev` for any Cloudflare
Workers project that depends on the SDK. `SDK_VERSION` is now a plain constant
in `src/version.js`, regenerated by the `npm version` lifecycle and guarded by
two tests: one asserts it matches `package.json`, one asserts nothing in `src/`
uses an import attribute.
